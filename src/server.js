...
import { pool, q, tx, logEvent } from './db.js';
...
const actorOf = (req) => (req.body?.actor || req.query.actor || 'anonymous').toString().slice(0, 40);
...
const isMember = async (roomId, actor) =>
...
app.get('/api/rooms/:id', async (req, res) => {
  const { rows: [room] } = await q('select * from rooms where id = $1', [req.params.id]);
  ...
  const [{ rows: members }, { rows: tasks }, { rows: events }] = await Promise.all([
    ...
    q(`select id, person_key, person, state, enrichment, draft, decision, claimed_by, linkedin_headline,
       ...
       from tasks where room_id = $1 order by created_at, id`, [req.params.id]),
    q(`select id, task_id, actor, type, data, created_at from events
       where room_id = $1 order by id desc limit 50`, [req.params.id]),
  ]);
  res.json({ room: { ...room, members }, tasks, events });
});

app.post('/api/rooms/:id/pause', async (req, res) => {
  const status = req.body.paused ? 'paused' : 'running';
  if (!(await isMember(req.params.id, actorOf(req))))
    return res.status(403).json({ error: `${actorOf(req)} is not a member of this room` });
  const { rows: [room] } = await q(
    'update rooms set status = $2 where id = $1 returning *', [req.params.id, status]);
  if (!room) return res.status(404).json({ error: 'no such room' });
...
app.post('/api/tasks/:id/:action', async (req, res) => {
  ...
  const { rows: [owner] } = await q('select room_id from tasks where id = $1', [id]);
  ...
  if (!ops[action]) return res.status(400).json({ error: `unknown action ${action}` });
  const [setSql, setParams, guard = '', guardParams = []] = ops[action];
  ...
  try {
    ...
    const { rows: [current] } = await q('select * from tasks where id = $1', [id]);
